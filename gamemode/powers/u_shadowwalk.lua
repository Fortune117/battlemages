...
function ACTIVE:cast( ply ) -- This should be the "meat" of the ability.
    ...
    ply:giveBuff( "buff_shadowwalk" )

    for k,v in pairs( ply.powers ) do
        if not v:canCast() then
            v:setCooldown( (v:getCooldown() - CurTime()) - 5 )
        end
    end
...
BM:addPower( "u_shadowwalk", ACTIVE, "active_base" )
